...
## Addendum (post-approval, user-requested — 2026-09-20)
**Show total env-step progress in the TUI.** The dashboard shows `iters N/M` but not the total step count. Add a `steps` line to the TIME panel (alongside iters/elapsed/eta) showing current/total timesteps (e.g. `steps 26,624 / 1,000,000`), sourced from SB3 `model.num_timesteps` and the configured `total_timesteps`, matching the panel's label/value formatting. This is a **cross-platform** display change (shown on macOS/Linux AND Windows) — a deliberate, user-approved exception to AC-9's "macOS/Linux byte-identical" rule for this one panel line; QA should verify the steps line renders the correct current/total, not flag it as an AC-9 regression. Everything else stays AC-9/AC-10 as specified.
