...
if ($DryRun) {
    Write-SetupLog 'DRY RUN - no network, install, or prompts. Planned actions:'
    Write-SetupLog "  repo root:              $RepoRoot"
    Write-SetupLog "  venv:                   $Venv (Python $SimPython)"
    Write-SetupLog "  torch pin (CUDA):       $TorchPin  via --index-url $TorchIndex"
    Write-SetupLog "  drones pin (train.sh):  $DronesRepo @ $DronesRef"
    Write-SetupLog '  1. ensure uv (prompt before installing into user space via astral.sh; never admin)'
...
if (-not $uvCmd) {
    Write-SetupLog 'uv is not installed. It is required to create the venv and install packages.'
    Write-SetupLog 'This installs uv into your user profile (no admin, no system-wide changes).'
    ...
    Write-SetupLog 'Downloading the uv installer from https://astral.sh/uv/install.ps1 ...'
    Invoke-RestMethod -Uri 'https://astral.sh/uv/install.ps1' -OutFile $installer
    Write-SetupLog 'Running the uv installer (user space) ...'
    & $installer
...
Write-SetupLog "Using uv: $($uvCmd.Source)"
...
if (-not $cudaLine) {
    Write-SetupLog 'WARNING: could not parse a "CUDA Version" from nvidia-smi output; continuing. The post-install torch.cuda check is the backstop.'
} else {
    ...
    Write-SetupLog "NVIDIA driver reports CUDA $cudaMajor.$cudaMinor (>= 12.x) - OK."
}

# --- 3. Create/reuse the uv venv. -------------------------------------------------------
$PythonExe = Join-Path $Venv 'Scripts\python.exe'
if (Test-Path -LiteralPath $PythonExe) {
    Write-SetupLog "Reusing existing venv '$Venv'."
} else {
    Write-SetupLog "Creating venv '$Venv' (Python $SimPython) ..."
...
Write-SetupLog "Installing $TorchPin from the CUDA wheel index ($TorchIndex) ..."
...
Write-SetupLog 'Installing pybullet==3.2.6 + numpy<2 + drone-fly (editable, [dev]) in one PyPI resolve ...'
...
Write-SetupLog 'Installing gym-pybullet-drones runtime deps explicitly (hand-maintained; the import check is the backstop) ...'
...
Write-Output ''
Write-SetupLog "Done. The '$Venv' venv has CUDA torch ($TorchPin) + the real-physics sim."
Write-SetupLog "Set 'device: cuda' in your train config (or omit it - resolve_device() auto-selects CUDA), then:"
Write-SetupLog "  $Venv\Scripts\drone-fly train --config configs\train\example.yaml"
Write-SetupLog 'Benchmark one iteration (watch the device log + SB3 time_elapsed / fps lines; compare to the ~821 s CPU / ~2.57 s-per-iter MPS baselines):'
Write-SetupLog "  $Venv\Scripts\drone-fly train --config configs\train\example.yaml   # config: device: cuda, small timesteps"
